...
def test_checkout_returns_url_for_pro_plan():
    with (
        ...
        patch("api.routes.billing.stripe.Customer.create") as mock_cc,
        patch("api.routes.billing.stripe.checkout.Session.create") as mock_sc,
    ):
        mock_cc.return_value = MagicMock(id="cus_new")
        mock_sc.return_value = MagicMock(url="https://checkout.stripe.com/pay/cs_test_abc")
...
def test_checkout_reuses_existing_stripe_customer():
    with (
        patch("api.routes.billing.get_settings", return_value=_mock_settings()),
        patch("api.routes.billing.session_context", return_value=_mock_session(_make_billing(stripe_customer_id="cus_existing"))),
        patch("api.routes.billing.stripe.Customer.create") as mock_cc,
        patch("api.routes.billing.stripe.checkout.Session.create") as mock_sc,
    ):
        mock_sc.return_value = MagicMock(url="https://checkout.stripe.com/pay/cs_test_xyz")
...
def test_portal_returns_url():
    with (
        ...
        patch("api.routes.billing.stripe.billing_portal.Session.create") as mock_ps,
    ):
