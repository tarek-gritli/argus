...
def test_scan_secrets_hardcoded_password():
    ...
    assert (
        next(h for h in hits if h.pattern_name == "hardcoded_password").severity_hint
        == Severity.MEDIUM
    )
...
def test_scan_sast_js_inner_html():
    ...
    assert (
        next(h for h in hits if h.rule_id == "INNER_HTML_ASSIGNMENT").severity_hint
        == Severity.MEDIUM
    )
...
def test_scan_dependencies_requirements_txt_vulnerable():
    ...
    assert len(hits) == 1
    assert hits[0].package == "jinja2"
    assert hits[0].cve_id == "CVE-2019-10906"
    assert hits[0].severity_hint == Severity.HIGH  # cvss 7.5 >= 7
...
def test_scan_dependencies_package_json_vulnerable():
    ...
    hits = _scan_dependencies(lines)
    assert len(hits) == 1
    assert hits[0].package == "lodash"
    assert hits[0].cve_id == "CVE-2020-8203"


def test_scan_dependencies_package_json_safe():
    lines = _lines(("package.json", 1, '"express": "^4.18.0"'))
    hits = _scan_dependencies(lines)
    assert hits == []
...
def test_detect_languages_python():
    diff = "+++ b/src/app.py\n+code"
    assert "python" in _detect_languages(diff)


def test_detect_languages_javascript():
    diff = "+++ b/src/app.js\n+code"
    assert "javascript" in _detect_languages(diff)
